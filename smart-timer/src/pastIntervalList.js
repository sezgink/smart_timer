...
function PastIntervalsList(props){
    ...
    {props.pastIntervalsInput.map((value,index) => (
      ...
      >
        ...
        <ListItemText primary={`${index+1}.Session ${converSecondsToHourMinutesSecondsString(value.intervalLength)}`} secondary={value.task} secondaryTypographyProps={{color: 'rgb(102, 157, 246)'}}/>
      </ListItem>
    ))}
    <li ref={scrollRef} />
   </List>);
